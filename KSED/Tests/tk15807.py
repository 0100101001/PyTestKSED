...
class KSEDbackTask(MPages, dataTest, KSEDLocators):
    ...
    def Creat(self,):
        ...
        self.titleCS.send_keys(u'Auto РД 15807 ' + dt)
    ...
    @allure.step("Вернуть задачу согласования")
    def backTask_RD(self):
        self.get(self._web_driver.current_url)
        self.backTasks.wait_to_be_clickable()
        self.backTasks.click()

        self.wait_page_loaded()
        try:
            self.takeTasks.wait_to_be_clickable()
        except:
            assert False, 'Кнопка забрать не появилась'