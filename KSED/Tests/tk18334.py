...
class KSEDInnerSoglAfterAddComment_RD(MPages, dataTest, KSEDLocators):
    ...
    def Creat(self,):
        ...
        self.titleCS.send_keys(u'Auto РД 18334 ' + dt)
    ...
    @allure.step("Проверка принятия решения с замечаниями")
    def AcceptInnerSoglWithComment(self):
        self.APPROVED_button.wait_to_be_clickable()
        self.APPROVED_button.click()

        assert "Вами были даны замечания по документу. Выполнение действия невозможно. Удалите замечания или измените решение" in self.infoMassage.get_text()

        self.btnOKpodpis.wait_to_be_clickable()  # Кнопка ОК информационного окна
        self.btnOKpodpis.click()

        self.butDelComment.wait_to_be_clickable() # Кнопка удаления комментариев
        self.butDelComment.click()

        self.confirm.wait_to_be_clickable()  # Кнопка удаления комментариев
        self.confirm.click()
        self.wait_page_loaded(wait_for_xpath_to_disappear='//div[@id = "message"]//span[@class = "wait"]')
