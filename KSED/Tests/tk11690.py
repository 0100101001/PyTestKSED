...
class KSEDsubordinate_doc(Locator, dataTest,KSEDLocators):
    ...
    def subordinate_doc(self):
        ...
        time.sleep(0.5)
        actions.move_to_element(page.expedition).move_by_offset(0, 10).click().perform()  # развернуть на "+"
        ...
        # page.chBinOnl.click()
        # time.sleep(2)
        # page.ActionTab.click()
        time.sleep(1)
        element = wait.until(EC.visibility_of_element_located((By.XPATH, KSEDLocators.subordinate)))
        #//////////////////////
        while element in page.subordinate:
            #for element in page.subordinate:
            element.click()
            try:
                actions.move_to_element(page.oneSubordInList).click().perform()#WebDriverWait(self.w, 1).until(EC.element_to_be_clickable((By.XPATH, KSEDLocators.oneSubordInList))):
                break
            except:
                continue
                print("No click")
                    #actions.move_to_element(page.oneSubordInList).click().perform()

        # assert WebDriverWait(self.w, 10).until(EC.visibility_of_element_located((By.XPATH, KSEDLocators.oneSubordInList)))
