...
class Edit_Password(Locator, dataTest, KSEDLocators):
    ...
    # Изменим пароль
    def edit_password(self, oldPas,newPas):
        page = Locator(self.w)
        ...
        page.edit_password.click()

        wait_page_loaded(self.w)

        assert "Изменить пароль пользователя" in self.w.title
        # Заполним поля ввода старого и нового пароля
        page.inputOldPassword.send_keys(oldPas)
        page.inputNewPassword1.send_keys(newPas)
        page.inputNewPassword2.send_keys(newPas)
        # Нажмем кнопку "ОК"
